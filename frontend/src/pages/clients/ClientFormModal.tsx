...
export default function ClientFormModal({
  ...
  async function onSubmit() {
    ...
    if (showAWG) {
      if (values.awgPublicKey) {
        clientPayload.publicKey = values.awgPublicKey;
      }
      if (values.awgAllowedIPs) {
        const allowedIPs = (values.awgAllowedIPs as string)
          .split(',')
          .map((s: string) => s.trim())
          .filter((s: string) => s !== '');
        if (allowedIPs.length > 0) {
                        ...
                        <>
                          <Form.Item>
                            <Typography.Text type="secondary">
                              {t('pages.clients.awgAutoGenHint', 'AmneziaWG: keys and IP are auto-generated by the server. The client config will be available in the subscription link.')}
                            </Typography.Text>
                          </Form.Item>
                        </>
                      )}
